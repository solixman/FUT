...
function DisplayPlayerCard(item) {
    let choosedelementid;
    choosedelementid = item.closest(".PlayerCard").children[0].id
    // console.log(choosedelementid)

filterdata(item.closest(".PlayerCard").children[0].id)


document.querySelector(".Thelist").addEventListener("click" , function(e){
    // console.log(choosedelementid)
   let content  = `${e.target.closest("#ThePlayer").innerHTML}`
   document.getElementById(choosedelementid).innerHTML = content
}) 
...
function filterdata(positionplayer){
    json.players.forEach(players => {
        if(players.position == positionplayer){
